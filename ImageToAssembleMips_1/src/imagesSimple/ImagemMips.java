...
public class ImagemMips {
    ...
    public static void main(String[] args) throws AWTException, URISyntaxException {
        try {
            ...
            int red, green, blue, linha, coluna;
          
            int corAtual = 0, corAnterior;
            JFrame janela = new JFrame();
            int tamanho = largura*altura;         
            int sequencia = 0, seq = 0;
            boolean cor;
            String hexa, addColuna = "";
            ...
            for(linha=0;linha<altura;linha++)
            {
                if(linha==0)
                {
                    ...
                    codigo += "#Linha: 1\n";
                    codigo += "\tadd $a0,$a0,1\t#linha inicial\n";
                    codigo += "\tadd $a1,$zero,1\t#coluna inicial\n";
                }
                else
                {
                ...
                for(coluna=0;coluna<largura;coluna++)
                {
                    ...
                    hexa = "0x"+Calcular(red) + Calcular(green) + Calcular(blue);
                    ...
                    addColuna += "\tadd $a2,$zero,"+hexa+"\n";
                    addColuna += "\tadd $a1,$a1," + 1 + "\t#coluna = " + (coluna+1) + "\n";
                    addColuna += "\tjal Plot\n";
                    ...
                    if(cor)
                    {
                        ...
                        codigo += "\tadd $s0,$zero,"+coluna+"\n";
                        codigo += "Imagem"+sequencia+":\n";
                        codigo += "\tbeq $a1,$s0,ImagemQuit"+sequencia+"\n";
                        codigo += "\tjal Plot\n";
                        codigo += "\tadd $a1,$a1,1\t#incrementa o valor\n";
                        codigo += "\tj Imagem"+sequencia+"\n";
                        codigo += "ImagemQuit"+sequencia+":\n";
                    }
                    else
                    {
                      codigo += addColuna;
