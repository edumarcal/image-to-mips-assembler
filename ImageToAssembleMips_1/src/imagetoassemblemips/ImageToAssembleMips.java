        ...
        try {
            ...
            for (int linha = 0; linha < altura; linha++) {
                if(linha==0)
                {
                    ...
                    codigo += "\tadd $a0,$a0," + 1 + "\t# modificar aqui linha\n";
                    codigo += "\tadd $a1,$a1," + 1 + "\t# modificar aqui coluna\n";
                    
                    codigo += "\t#Linha: " + String.valueOf(linha + 1) + "\n";
                    //codigo += "\tadd $a1,$a1," + 1 + "\t# coluna\n";
                    //codigo += "For_inicializacao_linha"+(linha+1)+":\n";
                    //codigo += "\tadd $a1,$a1," + 1 + "\t# coluna=1\n";
                    //codigo += "\tadd $s0,$zero,"+(largura)+"\t# tamanho da coluna\n";
                    //codigo += "For_codicao_linha"+(linha+1)+":\n";
                    //codigo += "\tble $a1,$s0,For_corpo_linha"+(linha+1)+"\n";
                    //codigo += "For_corpo_linha"+(linha+1)+":\n";
                }
                else
                {                  
                    codigo += "\t#Linha: " + String.valueOf(linha + 1) + "\n";
                    codigo += "\tadd $a0,$a0," + 1 + "\t# linha = "+(linha+1)+"\n";
                    codigo += "\tsub $a1,$a1," + (largura-1) + "\t# coluna = " +(linha+1) + "\n";
                  //  codigo += "\tadd $a1,$a1," + 1 + "\t# modificar aqui coluna\n";
                  //  codigo += "For_inicializacao_linha"+(linha+1)+":\n";
                  //  codigo += "\tadd $a1,$a1," + 1 + "\t# coluna=1\n";
                  //  codigo += "\tadd $s0,$zero,"+(largura)+"\t# tamanho da coluna\n";
                ...
                for (int coluna = 0; coluna < largura; coluna++) {
                    ...
                    {   
                        String hexa = "0x"+Calcular(red) + Calcular(green) + Calcular(blue);
                        codigo += "\tadd $s1,$zero," + qtdCorSequencial + "\t# Sequencia\n";
                        codigo += "\tadd $a2,$zero," + hexa + "\n";
                        //codigo += "For_corpo_linha"+(linha+1)+"_cor_"+hexa+"\n";
                        codigo += "\tble $a1,$s1,For_atualizacao_linha"+(linha+1)+"\t# salto para incremento\n";
                        

                        //codigo += "\tble $a1,$s0,For_corpo_linha"+(linha+1)+"\n";
                        //codigo += "\tjal Plot\n";
                        //codigo += "\tj For_atualizacao_linha"+(linha+1)+"\n";
                        //codigo += "\tadd $a1,$a1," + 1 + "\t# coluna\n";
                        //codigo += "For_inicializacao_linha"+(linha+1)+":\n";
                        //codigo += "\tadd $a1,$a1," + 1 + "\t# coluna=1\n";
                        //codigo += "\tadd $s0,$zero,"+(largura)+"\t# tamanho da coluna\n";
                ...
                codigo += "For_atualizacao_linha"+(linha+1)+":\n";
                codigo += "\tadd $a1,$a1,1\t# incremento da coluna\n";
                codigo += "\tjal Plot\n";
                //codigo += "\tj For_codicao_linha"+(linha+1)+"\n";
                //codigo += "\tjr $ra\n";
            }
